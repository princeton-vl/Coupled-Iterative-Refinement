...
@gin.configurable
def load_efficientnet(coarse_run_id):
    run_dir = EXP_DIR / coarse_run_id
    run_id = coarse_run_id
    run_dir = EXP_DIR / run_id
    model = EfficientNet.from_name('efficientnet-b3', in_channels=6)
    model.n_features = 1536

    ckpt = torch.load(run_dir / 'checkpoint.pth.tar')
    print(f"Loaded EfficientNet model: {run_dir / 'checkpoint.pth.tar'}")
    state_dict = ckpt['state_dict']
    state_dict = {k.replace('backbone.',''):v for k,v in state_dict.items()}
    pose_fc = nn.Linear(1536, 9, bias=True).cuda().eval()
    pose_fc_state_dict = {"weight": state_dict.pop('pose_fc.weight'), "bias": state_dict.pop('pose_fc.bias')}
    pose_fc.load_state_dict(pose_fc_state_dict, strict=True)
    model.load_state_dict(state_dict, strict=True)
    model = model.cuda().eval()

    def run_efficientnet(image_inputs, mrcnn_pose, K_cropped):
        x = model(image_inputs)
        x = x.flatten(2).mean(dim=-1)
        coarse_pose = pose_fc(x)
        dR = compute_rotation_matrix_from_ortho6d(coarse_pose[:, 0:6])
        vxvyvz = coarse_pose[:, 6:9]
        return apply_imagespace_predictions(mrcnn_pose, K_cropped, vxvyvz, dR)

    return run_efficientnet
