...
def main():
    ...
    parser.add_argument('--ds_workers', type=int, default=len(os.sched_getaffinity(0))) # This command doesn't work on Mac or Windows
...
def train(rank, world_size, args, train_metrics, val_metrics, batch_size):
    colored_traceback.add_hook()
    gin.parse_config_files_and_bindings(["configs/base.gin", f"configs/train_{args.dataset}_{'rgb' if args.rgb_only else 'rgbd'}.gin"], args.override)
            ...
            if (global_step > 0 and ((global_step % gin_globals().val_freq) == 0)) or args.evaluate:
                with torch.no_grad():
                    ...
                    if rank == 0:
                        ...
                        if not args.evaluate:
                            torch.save(model.state_dict(), f'checkpoints/{global_step}.pth')
                            torch.save(optimizer.state_dict(), f'checkpoints/{global_step}_optimizer.pth')
                            torch.save(scheduler(global_step).state_dict(), f'checkpoints/{global_step}_scheduler.pth')
...
def forward_pass(model, data, generator, detector, use_detector, val_mode=False, num_inner_loops=40, num_solver_steps=10, debug=False):
    ...
    for key in {'TWO', 'T0O', 'bbox', 'id_in_segm', 'diameter_m', 'symmetries'}:
        obs['objects'][key] = torch.as_tensor(obs['objects'][key], device='cuda')
